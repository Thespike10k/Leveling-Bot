...
export async function syncEmojis(token) {
    if (!token) {
    ...
    try {
        if (!fs.existsSync(emojisPath)) {
            console.log(`${tag} ${col.red}emojis.json not found at ${emojisPath}${col.reset}`);
            return;
        }
    ...
    let updated = false, skipped = 0, uploaded = 0, fixed = 0, failed = 0;
    const result = { ...emojis };

    for (const [key, emojiStr] of emojiEntries) {
        if (typeof emojiStr !== 'string') continue;
        ...
        if (existing) {
            const correct = animated ? `<a:${existing.name}:${existing.id}>` : `<:${existing.name}:${existing.id}>`;
            if (emojiStr !== correct) {
                result[key] = correct;
                updated = true;
                fixed++;
        ...
        try {
            ...
            const newEmoji = await rest.post(`/applications/${appId}/emojis`, { body: { name, image } });

            result[key] = animated ? `<a:${newEmoji.name}:${newEmoji.id}>` : `<:${newEmoji.name}:${newEmoji.id}>`;
            updated = true;
            uploaded++;
    ...
    const parts = [
        skipped === 1 ? `${col.green}${skipped} present${col.reset}` : (skipped > 0 ? `${col.green}${skipped} present${col.reset}` : null),
        fixed === 1 ? `${col.yellow}${fixed} fixed${col.reset}` : (fixed > 0 ? `${col.yellow}${fixed} fixed${col.reset}` : null),
        uploaded === 1 ? `${col.cyan}${uploaded} uploaded${col.reset}` : (uploaded > 0 ? `${col.cyan}${uploaded} uploaded${col.reset}` : null),
        failed === 1 ? `${col.red}${failed} failed${col.reset}` : (failed > 0 ? `${col.red}${failed} failed${col.reset}` : null),
    ].filter(Boolean);
